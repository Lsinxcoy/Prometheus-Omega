...
class DopamineWriteGate:
    """多巴胺写入门控 - 宪法机制"""
    def __init__(self, threshold: float = 0.3, dopamine_min: float = 0.2):
        self.threshold = threshold
        self.dopamine_min = dopamine_min
    ...
    def can_write(self, importance: float, utility: float, veracity: float, dopamine: float) -> bool:
        ...
        quality = importance * utility * veracity * dopamine
        allowed = quality >= self.threshold and dopamine >= self.dopamine_min
        if allowed:
            self.total_allowed += 1
        return allowed
    
    def get_stats(self) -> dict:
        return {"total_attempts": self.total_attempts, "total_allowed": self.total_allowed}

class AntiEvolutionGate:
    """反演化门控 - 防止系统退化"""
    def __init__(self, min_eval: float = 0.7):
        self.min_eval = min_eval
        self.blocked_count = 0
    
    def can_evolve(self, eval_result: float) -> bool:
        allowed = eval_result >= self.min_eval
        if not allowed:
...
class WeibullForgetting:
    """威布尔遗忘曲线"""
    def __init__(self, scale: float = 7.0, shape: float = 1.5):
        self.scale = scale
        self.shape = shape
    
    def get_retention(self, age_days: float) -> float:
        import math
        return math.exp(-(age_days / self.scale) ** self.shape)
