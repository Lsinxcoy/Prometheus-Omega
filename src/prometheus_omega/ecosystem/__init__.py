...
class LotkaVolterra:
    ...
    def add_species(self, name: str, initial_pop: float, 
                   growth: float, capacity: float) -> None:
...
class HarnessX:
    ...
    def hook(self, hook_name: str, func: callable) -> None:
...
class ExperienceRecall:
    ...
    def add_step(self, trajectory_id: int, step: Dict) -> None:
        """添加步骤到轨迹"""
        if trajectory_id < 0:
...
class MARS:
    ...
    def update(self, belief: str, value: float, evidence_weight: float = 1.0) -> None:
        """更新信念
...
class CommunityTree:
    ...
    def _collect_leaves(self, node: CommunityNode, leaves: list) -> None:
        if node.is_leaf:
            leaves.append(node)
        else:
            for child in node.children:
...
def detect_communities(store: Any, branch: str = "main",
                       min_size: int = 3,
...
def detect_hierarchical(store: Any, branch: str = "main",
...
def _split_recursive(store: Any, parent: CommunityNode,
...
def _build_adjacency(store: Any, node_ids: set[str],
...
def _bfs_components(store: Any, node_ids: set[str],
...
def find_bridges(store: Any, communities: list[list[str]],
...
def compute_modularity(store: Any, communities: list[list[str]],
